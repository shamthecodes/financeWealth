...
export const checkBudgetAlert = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    // Process budgets in parallel with error handling
    await Promise.all(
      budgets.map(async (budget) => {
        try {
          await step.run(`check-budget-${budget.id}`, async () => {
            const defaultAccount = budget.user.accounts[0];
            if (!defaultAccount) return; //skip if no default
            ...
            if (
              percentageUsed >= 80 && //changes i done
...
export const triggerRecurringTransactions = inngest.createFunction(
  ...
  async ({ step }) => {
      ...
      async () => {
        return await db.transaction.findMany({
          where: {
            ...
            status: "COMPLETED",
            OR: [
              { lastProcessed: null }, //Never procced
    ...
    //2. Crete events for each transaction
...
function isTransactionDue(transaction) {
  //If no lastProcessed date, transacion is due
